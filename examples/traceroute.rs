...
use traceroute_rust::traceroute::probe::generator::{IcmpProbeTaskGenerator, UdpProbeTaskGenerator};
use traceroute_rust::traceroute::probe::parser::{IcmpProbeResponseParser, ProbeReplyParser, UdpProbeResponseParser};
use traceroute_rust::traceroute::Traceroute;

#[tokio::main]
async fn main() -> io::Result<()> {
    let parser = ProbeReplyParser::UDP(UdpProbeResponseParser);
    let ip_addr = IpAddr::V4(Ipv4Addr::from_str("216.58.204.238").unwrap());

    let mut icmp_sniffer = IcmpProbeResponseSniffer::new(parser)?;

    let generator = Box::new(UdpProbeTaskGenerator::new(33434));
